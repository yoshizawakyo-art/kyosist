...
""")
    ...
    if test_queries:
        best_iter = max(history, key=_test_passed_count).get("iteration")
    else:
        best_iter = max(history, key=_train_passed_count).get("iteration")
...
def _test_passed_count(history_item: dict) -> int:
    return history_item.get("test_passed") or 0


def _train_passed_count(history_item: dict) -> int:
    return history_item.get("train_passed", history_item.get("passed", 0))
